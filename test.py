...
parser.add_argument("--data_path", type=str, default="datasets/test/Rain100L/rainy_images", help='path to training data')
...
def main():
    ...
    if opt.use_GPU:
        model = model.cuda()
    model.load_state_dict(torch.load(os.path.join(opt.logdir, 'checkpoint.pth')))
    model.eval()
    ...
    for img_name in os.listdir(opt.data_path):
        if is_image(img_name):
            ...
            b, g, r = cv2.split(y)
            y = cv2.merge([r, g, b])
            #y = cv2.resize(y, (int(1024), int(1024)), interpolation=cv2.INTER_CUBIC)
